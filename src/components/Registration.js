...
export function Registration() {
    ...
    async function handleSubmit(event) {
        ...
        const response = await fetch("https://k4backend.osuka.dev/users", requestOptions)
            .then((res) => {
                return res.json()
            }).then((data) => {
                if (data.status === 200) {
            ...
            < div className="w-full max-w-xs" >
                <form onSubmit={handleSubmit} className="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" placeholder="Email" onChange={e => setUserRegEmail(e.target.value)} class="w-full px-4 py-2 mt-2 border rounded-md focus:outline-none focus:ring-1 focus:ring-blue-600" />
                    </div>
